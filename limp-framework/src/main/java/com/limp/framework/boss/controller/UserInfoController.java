...
import com.limp.framework.auth.bean.DES;
...
//    @Transactional  事物
    ...
    public String updateUserInfoState(@PathVariable("id") String id,@PathVariable("state") String state) {
        ...
        userInfo.setId(id);
        userInfo.setState(Constant.STATE_DELETE);
    ...
    public String selectUserInfoList(UserInfo userInfo,Integer rememberme,HttpServletRequest request,HttpServletResponse response) {
        //参数校验
        ...
        try {
            ...
            loginMap.put("type", user.getUtype());
            /*******************test*****************************/
            //授权的权限控制
            //====布尔值判断
            //功能权限
            ...
            if(rememberme != null && rememberme == 1) {
                ...
                String ck=DES.encrypt(JsonUtils.toJson(map),"password");
                Cookie cookie = new Cookie("rememberme", ck);
                ...
                Cookie cookies[] = request.getCookies();
                    ...
                    for (int i = 0; i < cookies.length; i++) {
                        {
                            Cookie cookie = new Cookie("rememberme", "");//这边得用"",不能用null
    ...
    public String userInit(HttpServletResponse response,HttpServletRequest request) {
        //用户【账号、密码】登录认证
        Map map=new HashMap();
        Cookie cookies[] = request.getCookies();
        if (cookies != null) {
            for (int i = 0; i < cookies.length; i++) {
                if (cookies[i].getName().equals("rememberme")) {
                    try {
                        String accountpwd=cookies[i].getValue();
                        try {
                            String  accountpwdY=DES.decrypt(accountpwd,"password");
                            map=JsonUtils.convertJsonStrToMap(accountpwdY);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    } catch (Exception e) {
        ...
        return Result.getInstance("200","succes",map,"").getJson();
    }
