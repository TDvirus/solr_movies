...
var sysCode={
			...
			methods:{
				...
				handleSeach:function(){
					...
					if(!system.isBlank($("#perms_serach_code").val())){
						param['dictValue']=$("#perms_serach_code").val()+"";
					}

                    if(!system.isBlank($("#perms_serach_name").val())){
                        param['dictName']=$("#perms_serach_name").val();
                    }
		...
		$("#"+sysCode.cons.tableId).LPClickDelegate(".del",function(){
			...
			system.confirm("",{title:"删除提示"},function(index){
				system.post("/sys/code/"+delId,{type:2},function(data){
					if(data.code=="200") {
						system.tip("删除成功",1);
						sysCode.initDomainTable({pid:system.isBlank(sysCode.clickTreeId)?"0":sysCode.clickTreeId});
						//重新加载树形结构
						sysCode.initTree();
					}else{
						system.tip("删除失败，请重试！",0)
	...
	initTree:function(){
		...
		function onClick(event, treeId, treeNode, clickFlag) {
			if(1==clickFlag){
                ...
                console.log('========treeid========')
				sysCode.initDomainTable({pid:treeNode.id});

			}
		}
		var nodes=new Array();
		system.post("/sys/codeList/",{type:4,rows:10000},function(data){
			for(var i in data.result){
				...
				node["id"]=domain.id+"";
				node["pId"]=domain.pid+"";
				node["pvalue"]=domain.dictPvalue+"";
				node["name"]=domain.dictName+"";
	...
	initDomainTable:function(params){
		var option={
			...
			colModel : [
				...
				{key :'dictName',name : '码表名称'},
				{key :'dictValue',name : '码表编号',sort:'dictValue'},
				{key :'dictKey',name : 'key'},
	...
	saveDomain:function(id){
		system.open("<div id='lp'>"+$('#tkAddCode').html()+"</div>",{title:system.isBlank(id)?"新增码表":"修改码表",area:["630px","300px"]},function(layern,index){
					...
					setTimeout(function(){
						...
						if(system.isBlank(id)){
							...
							if(sysCode.clickTreeId){
								...
								codeObj={dictPvalue:obj.dictValue,parentName:obj.dictName,pid:obj.id,dictKey:obj.dictKey}
							}else {
								codeObj={dictPvalue:"0",parentName:"一级码表",pid:"0"}
							}
						}else{
							codeObj=system.cache[id];
							var pname=system.isBlank(system.cache[codeObj.pid])?"一级目录":system.cache[codeObj.pid].dictName;
