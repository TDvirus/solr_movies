				   ...
				   var  arrIds= $("#"+sysCommand.cons.tableId).LPTableCheckedIds();
					...
					system.confirm("",{title:"删除提示"},function(index){
						system.post("/system/command/"+arrIds.join(","),{type:2},function(data){
							if(data.code=="200") {
								system.tip("删除成功",1);
								sysCommand.initDomainTable({rc2:sysCommand.clickTreeId});
							}else{
				...
				layui.use(['form','jquery',"laydate"], function() {
					...
					setTimeout(function(){
						...
						form.on('submit(addFilterSubmit)', function(data) {
							...
							params["type"]=1;
							params["rc2"]=sysCommand.clickTreeId;
							//异步新增人员
    ...
    initTree:function(){
        ...
        function onClick(event, treeId, treeNode, clickFlag) {
            if(1==clickFlag){
                ...
                sysCommand.clickTreeId=treeNode.id;
                sysCommand.initDomainTable({rc2:sysCommand.clickTreeId});
            }
        }
        var nodes=new Array();
        system.post("/system/commandCates",{type:4,rows:200},function(data){
